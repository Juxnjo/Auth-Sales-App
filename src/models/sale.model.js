...
class SaleModel {
    ...
    static async createSale({ producto, cupo_solicitado, franquicia, tasa, usuario_creacion }) {
        try {
            let tasaFinal = tasa;
            if (producto === "Tarjeta de Credito") {
                tasaFinal = 0; 
            } else if (["Credito de Consumo", "Libranza Libre Inversión"].includes(producto) && !tasa) {
            ...
            const result = await pool.query(
                ...
                [producto, cupo_solicitado, franquicia || null, tasaFinal, usuario_creacion]
            );
    ...
    static async updateSale(id, { producto, cupo_solicitado, franquicia, tasa,      usuario_actualizacion }) {
        try {
            let tasaFinal = tasa;

            // Validaciones según el producto
            if (["Credito de Consumo", "Libranza Libre Inversión"].includes(producto)) {
                if (!tasa && tasa !== 0) {
            ...
            let franquiciaFinal = producto === "Tarjeta de Credito" ? franquicia : null;
            ...
            const values = [producto, cupo_solicitado, franquiciaFinal, tasaFinal, usuario_actualizacion, id];
